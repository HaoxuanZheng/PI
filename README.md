...
Milestones through **Alpha hardening** (V0.22) are implemented: per-route rate limits with 429s (`0020`), edge-tombstoning object deletion plus a 7-day account deletion request window (`0021`), full user-owned account export (`0022`), onboarding state machine with privacy-safe product analytics (`0023`), security headers with structured failure logging (`0024`), least-privilege database roles proven by first real test execution (`0025`), account purge execution after the recovery window (`0026`), launch readiness with a database-backed readiness probe, backup policy, and launch checklist, idempotency keys on retried object creation (`0028`), and a per-user encrypted provider connection vault preferred over operator tokens (`0029`), and Google OAuth consent with transparent token refresh (`0030`).
